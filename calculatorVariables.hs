...
cTheta = aa :@ aa where aa = Lam "a" $ Lam "b" $ b :@ (a :@ a :@ b)
...
-- buildAbstractor :: [String] -> [(Expr -> Expr)]
-- buildAbstractor []       = []
-- buildAbstractor (x : xs) = Lam x : buildAbstractor xs

-- separateAbstractorBody :: [Char] -> (String, String)
-- separateAbstractorBody ""                 = ("", "")
-- separateAbstractorBody ('-' : '>' : body) = ("", body)
-- separateAbstractorBody (x         : xs  ) = ([x] ++ y, ys)
--   where (y, ys) = separateAbstractorBody xs

-- buildLambda :: [Expr -> Expr] -> Expr -> Expr
-- buildLambda []       body = body
-- buildLambda (l : ls) body = l $ buildLambda ls body

-- instance Read Expr where
--   readsPrec _ ('(' : xs) =
--     [ (m :@ n, rest) | (m, ')' : ys) <- reads xs, (n, rest) <- reads ys ]
--   readsPrec _ ('\\' : xs) = [(lambdaExpression, "")]
--    where
--     (abstractor, body) = separateAbstractorBody xs
--     lambdaAbstractor   = buildAbstractor $ splitOn " " abstractor
--     lambdaBody         = read body :: Expr
--     lambdaExpression   = buildLambda lambdaAbstractor lambdaBody
--   readsPrec _ xs =
--     [ (x :@ y, rest) | (x, ys) <- reads xs, (y, rest) <- reads ys ]
--   -- readsPrec _ x = [(Var x, "")]


-- read "\\x1 x2 -> x1 x2 x2" == Lam "x1" (Lam "x2" (Var "x1" :@ Var "x2" :@ Var "x2"))
-- read "x y z" == (Var "x" :@ Var "y" :@ Var "z")
-- test xs = buildLambda $ splitLambdaExpression xs
check :: Expr -> Bool
...
t2 :: Expr
t2 = Var "x" :@ Var "y" :@ Var "z"

t3 :: Expr
t3 = Var "x" :@ Var "y" :@ Var "z"

t4 :: Expr
t4 = Var "x" :@ (Var "y" :@ Var "z")

t5 :: Expr
t5 = Lam "x" (Var "x" :@ Var "y")

t6 :: Expr
t6 = Lam "x" (Lam "y" (Var "x" :@ Var "y"))

t7 :: Expr
t7 = Lam "x1" (Lam "x2" (Var "x1" :@ Var "x2" :@ Var "x2"))

t8 :: Expr
t8 =
  let x = Var "x"
      f = Var "f"
  in  Lam "f" (Lam "x" (Lam "x" f :@ (x :@ x)) :@ (Lam "x" $ f :@ (x :@ x)))

t8' :: Expr
t8' =
  let x = Var "x"
      f = Var "f"
  in  Lam "x" (Lam "x" f :@ (x :@ x))

t9 :: Expr
t9 = x :@ x where
  x = Lam "a" $ Lam "b" $ Lam "b" b :@ (a :@ a :@ b)
  a = Var "a"
  b = Var "b"

test1 = "\\f x y z f d s f g h r e -> x"
test2 = "\\f x -> (\\m -> n) x"
test3 = "\\f -> (\\x -> (\\x -> f) (x x)) (\\x -> f (x x))"

-- main' = test test3

-- Lam "f" (Lam "x" (Lam "x" (Var "f") :@ (((Var "x" :@ Var "x") :@ Var "))") :@ Lam "x" (Var "f" :@ ((Var "x" :@ Var "x") :@ Var "))")))))
-- Lam "f" (Lam "x" (Lam "x" (Var "f") :@ (Var "x" :@ Var "x")) :@ Lam "x" (Var "f" :@ (Var "x" :@ Var "x")))